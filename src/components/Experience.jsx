      ...
      <div className="mx-auto max-w-4xl px-4">
        ...
        <div className="flex flex-col space-y-12">
          {EXPERIENCES.map((exp, index) => (
            ...
            >
              <div className="w-fluu text-sm font-semibold text-stone-300 md:w-1/4 lg:text-lg">
                {exp.yearRange}
              </div>
              <div className="w-full md:w-3/4">
                <h3 className="mb-2 text-lg lg:text-2xl">
                  {exp.role} -{" "}
                  <span className="bg-gradient-to-b from-blue-400 to-green-200 bg-clip-text text-transparent">
                    {exp.company}
                  </span>
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {exp.techStack.map((tech, id) => (
                    <span
                      key={id}
                      className="rounded-full border-2 border-green-500/60 px-3 py-1 text-sm font-semibold"
                    >
                      {tech}
                    </span>
                  ))}
